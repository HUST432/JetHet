...
#include "Buffer.h"
#include <conio.h>
NetInteraction::NetInteraction(Configuration& conf)
{
	this->conf = &conf;
...
void NetInteraction::onReceiveMessage(SOCKET client, string msg)
{
	...
	Buffer& buffer=rh.GetResponse(head);
	//console.Warning(string(buffer));
	server->SendBuffer(client,buffer.buf,buffer.size);
	delete& buffer;
